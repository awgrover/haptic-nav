...
boolean encoder_begin() {
  static After done_with_show(500ul);
  ...
  if (! began ) {
    ...
    encoder_neo.show();
    Serial << F("encoder begin, discrete pixes ") << EncoderPixCount << endl;

  }

  return done_with_show( [](){ 
    encoder_neo.clear();
    encoder_neo.show();
  });
}
...
void show_encoder() {
  ...
  if ( value != last ) {
    ...
    encoder_neo.show();
    Serial << value << F(" ") << _HEX(distance_colors[ value ]) << endl;
    last = value;
  }
}
